...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction — proces ręczny jest wykonalny, ale męczący i podatny na błędy przy wielu produktach naraz"
    - topic: "insight"
      decision: "status zamówienia powinien być wyliczany ze statusów produktów, nie ustawiany ręcznie — ręczny arkusz tego nie robi"
    - topic: "persona scope"
      decision: "pojedynczy nazwany użytkownik — sam autor projektu, jedno konto"
    - topic: "access control"
      decision: "login (email + hasło), model płaski — brak ról"
    - topic: "flow scope — zwrot_otrzymany"
      decision: "dodany do modelu danych, ale POZA głównym przepływem dowodzącym wartość (przepływ kończy się na zwrot_wysłany); zwrot_otrzymany to Secondary success criterion, niższy priorytet"
  frs_drafted: 8
...
> Insight (skala 100x): sama reguła wyliczania statusu zamówienia skaluje się bez zmian — nie zależy od liczby użytkowników. To, co by wymagało pracy przy 100x skali, to model dostępu: dziś zakładamy jednego, stałego użytkownika (login bez rejestracji), a przy wielu userach trzeba by dodać pełny support rejestracji kont. Nieistotne dla obecnego zakresu — zapisane jako obserwacja, nie zmiana scope'u.
...
- FR-001: User can log in with email and password. Priority: must-have
  > Socrates: Counter-argument considered: "single-user app — login is an unnecessary layer since only one person ever uses it." Resolution: kept; login stays as the visible access-control mechanism, a deliberate choice locked in Phase 2 (Access Control), not a technical necessity for a single user.
- FR-002: User can create a new order with a store, a date, and one or more products (each with brand, type, and price required; description and category optional at creation, editable later). Priority: must-have
  > Socrates: Counter-argument considered: "requiring all fields upfront is too rigid — description/category aren't always known yet." Resolution: revised; description and category are now optional at creation and can be filled in later, reducing friction when adding an order.
...
- FR-004: User can delete an order. Priority: must-have
  > Socrates: Counter-argument considered: "deleting without confirmation could lose data that P1 statistics would need later." Resolution: kept; P1 statistics are out of this evening's scope entirely, so losing that data isn't a live blocker. No confirmation step added — deliberate simplicity for a single-user tool.
- FR-005: User can view a list of all orders, each showing a status computed automatically from its products' statuses, grouped/sorted so unresolved orders surface before finished ones. Priority: must-have
  > Socrates: Counter-argument considered: "without sorting/filtering by status, the list becomes unreadable as orders accumulate." Resolution: revised; the list groups/sorts unresolved orders first — a cheap default-ordering addition, not a new filtering feature.
- FR-006: User can set a product's status to "kept" or "to be returned" (reversible: kept → to be returned). Priority: must-have
  > Socrates: Counter-argument considered: "unlimited reversal of the decision could mask mistakes instead of surfacing them." Resolution: kept; unrestricted reversal was a deliberate requirement from Vision & Problem (Phase 1) — the user explicitly changes their mind sometimes, and no audit trail is needed for this MVP.
- FR-007: User can mark a product's status as "return shipped", reversible back to "to be returned". Priority: must-have
  > Socrates: Counter-argument considered: "without an undo path, an accidental mark leaves the product stuck in the wrong state." Resolution: revised; added a reverse transition (return shipped → to be returned) symmetric to the one already allowed for "kept" — a cheap addition, one more allowed edge in the state machine.
- FR-008: User can mark a product's status as "return received". Priority: nice-to-have
  > Socrates: Counter-argument considered: "no counter-argument; it stands as written." Resolution: kept as nice-to-have, unchanged.
...
- Brak statystyk i wykresów (per kategoria/marka/miesiąc) — celowo odłożone poza ten zakres (P1, przyszła rozbudowa).
